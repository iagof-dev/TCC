...
export default function Search(props) {
	...
	async function handleSearch(e) {
		...
		const booksByAuthor = groupBooksByCode(await Api.books.getBookByAuthor(formData.author))

		const booksByTitleData = await Api.books.getBookByTitle(formData.title)

		const booksByTitle = booksByTitleData.length > 0? groupBooksByCode(booksByTitleData) : false
